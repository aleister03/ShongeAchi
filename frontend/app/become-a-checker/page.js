...
export default function CheckerSignupPage() {
  ...
  async function handleSubmit(event) {
    ...
    setSubmitting(true);
    try {
      const [nidPhotoBase64, profilePhotoBase64] = await Promise.all([
        fileToBase64(files.nidPhoto),
        fileToBase64(files.profilePhoto),
      ]);

      await api.post("/api/checkers/signup", {
        name: form.name.trim(),
  ...
  if (submitted) {
    return <main className="grid min-h-screen place-items-center bg-[linear-gradient(145deg,#eaf4e4,#c6ded8)] px-5 py-12">
      <section className="w-full max-w-xl rounded-3xl bg-white p-8 text-center shadow-xl sm:p-12">
        ...
        <p className="mt-4 leading-7 text-gray-600">Thank you, {form.name}. Your application, service area, and documents have been submitted for admin review. Your account will remain unable to receive elder assignments until an admin explicitly approves it.</p>
        <div className="mt-7 rounded-2xl bg-[#eef8ea] p-4 text-sm text-[#2a5a4a]">You can sign in once your application is approved — you'll be notified at the phone number you registered with.</div>
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <form onSubmit={handleSubmit} noValidate className="overflow-hidden rounded-3xl bg-white shadow-xl">
        ...
        <div className="space-y-10 px-6 py-8 sm:px-10">
          ...
          <section><h2 className="mb-2 text-xl font-bold text-[#234f42]">Identity documents</h2><p className="mb-5 text-sm text-gray-600">Both images are required for verification and are submitted directly to the server for admin review.</p><div className="grid gap-6 sm:grid-cols-2">
          ...
          {submitError && <div className="rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-700" role="alert">{submitError}</div>}

          <button type="submit" disabled={submitting} className="w-full rounded-full bg-[#2a7a5a] px-8 py-4 font-semibold text-white transition hover:bg-[#215f49] focus:outline-none focus:ring-4 focus:ring-[#2a7a5a]/20 disabled:opacity-50">
