...
/**
 * Reusable chat thread for the Communication System (a Premium feature —
 * see backend lib/subscription.js). Used by both the family "Messages" tab
 * and the checker portal. Supports an optional single image/video
 * attachment per message (base64 data URL — no object storage configured
 * in this stack).
 *
 * No websocket infra in this stack, so this polls every 5s rather than
 * pushing updates live — good enough for a capstone demo.
 */
export default function MessageThread({ elderId, role, familyMemberId, checkerId, myName, otherPartyLabel }) {
  const [messages, setMessages] = useState(null);
  const [gateInfo, setGateInfo] = useState(null); // { isPremium, hasCheckerAssigned }
