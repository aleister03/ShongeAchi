...
// Headline: concern level, score, trend and what it was based on. Shared
// between the wellbeing page and the checker's post-visit screen.
...
// The evidence behind the score. Collapsed by default.
export function ConcernSignals({ signals }) {
  if (!signals || !signals.windowComparison) return null;

  const {
    windowComparison: window, attendance, adherence, streaks, wellbeingSlope,
    moodShift, mobilityShift, appetiteShift, engagementShift,
    followUpsRequested, latestVisit, recentObservations, visitsAnalyzed,
  } = signals;
...
// Presentational half of the card, kept separate from data fetching so
// every state (loading, empty, AI, fallback, limited history) can be
// rendered and verified directly from a payload.
...
// Shown in place of the card when the elder is on the Free plan.
export function PremiumUpsell({ message, elderId }) {
  return (
    <Card>
      <div className="elderItem">
        <strong>Concern Assessment</strong>
      ...
      <Link href={`/elder/${elderId}/subscription`} className="pillButton" style={{ marginTop: 12 }}>
        Upgrade this elder
      </Link>
    </Card>
  );
...
// familyMemberId / checkerId: same caller-supplied-id trust model as the
// rest of this backend — exactly one of these should be passed depending
// on who's viewing.
export default function ConcernAssessmentCard({ elderId, familyMemberId, checkerId, canRecompute }) {
  ...
  const load = useCallback(() => {
    api
      .get(`/api/wellbeing/${elderId}/ai-assessment?${idParam}`)
  ...
  const recompute = () => {
    ...
    setError("");
    api
      .post(`/api/wellbeing/${elderId}/ai-assessment`, { checkerId })
      .then(load)
      .catch((err) => setError(err.message))
      .finally(() => setRecomputing(false));
  };

  if (premiumRequired) return <PremiumUpsell message={premiumRequired} elderId={elderId} />;
