...
// Leaflet's default marker icon references image files via a relative
// path that breaks under Next.js/webpack bundling — point it at the CDN
// copies instead. This is the standard workaround for react-leaflet + Next.
...
/** Re-fits the map's view whenever the set of points changes. */
 ...
 * records that actually have geocoded coordinates. Must be loaded with
 * `next/dynamic` + `{ ssr: false }` since Leaflet requires `window`.
 *
 * @param {object} elder - the selected elder (needs address.coordinates)
 * @param {Array} recommendations - array of { checker, distanceKm } from
 *   GET /api/assignments/recommend
 */
