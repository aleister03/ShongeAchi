  ...
  return (
    <main className="min-h-screen" style={{ background: "#FBF3D9" }}>
      ...
      <div className="px-10 py-10 max-w-3xl mx-auto">
        ...
        <Section
          title="Disaster Mode"
          description="While active, EVERY elder's missed-check-in escalation window is capped at the value below, overriding individual settings (it can only tighten the window, never loosen it). Use during cyclone warnings or other emergencies. Applies on the next Escalation Engine sweep."
        >
          ...
          />
          <div className="mt-2">
            <label className="block text-xs font-medium text-gray-600 mb-1">Note (shown on the admin dashboard banner)</label>
            <input
              value={config.disasterMode.note}
              onChange={(e) => update("disasterMode.note", e.target.value)}
              placeholder='e.g. "Cyclone Remal warning — Dhaka"'
              className="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-[#2a7a5a]"
            />
          </div>
        </Section>
        ...
        <Section
          ...
          description="Where the AI-Powered Concern Metrics dashboard draws the Stable / Elevated / Critical lines. Applies to every wellbeing endpoint immediately."
        >
          <NumberField
        ...
        <Section title="Premium Pricing" description="Reference values for the public Pricing page and future payment integration.">
          <NumberField
        ...
        <Section
          title="Notifications"
          description="Controls whether the Escalation Engine sends real notifications (in-app + email via notifyFamily) when it raises an escalation. Turning this off still raises the Escalation record — it just skips notifyFamily()."
        >
          <label className="flex items-center justify-between py-2">
        ...
        <Section title="Supported Service Areas" description="Reference list for future area pickers on signup/registration forms.">
