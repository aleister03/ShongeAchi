...
// -----------------------------------------------------------------------
// Communication System — Premium subscribers can message their assigned
// checker directly (spec: "regarding appointments, medication reminders,
// or special instructions"). CHANGED: now gated on the real subscription
// object (see lib/subscription.js's isPremium()) backed by a local payment
// payments, instead of the old manually-set isPremium boolean flag.
//
// Same caller-supplied-id trust model as the rest of this backend: no
// verified session, relationship is checked against familyMemberId /
// assignedCheckerId the same way the wellbeing routes already do.
// -----------------------------------------------------------------------

// GET /api/messages/[elderId]?role=family&familyMemberId=...
// GET /api/messages/[elderId]?role=checker&checkerId=...
export async function GET(request, context) {
  try {
    ...
    return NextResponse.json(
      { success: true, data: { messages, isPremium: isPremium(elder), hasCheckerAssigned: !!elder.assignedCheckerId } },
      { status: 200 }
    );
  } catch (error) {
...
// Body: { role: "family"|"checker", familyMemberId?, checkerId?, senderName, text?, attachment? }
// attachment (optional): { kind: "image"|"video", data: "<base64 data URL>", mimeType: "image/jpeg" }
const MAX_IMAGE_BYTES = 5 * 1024 * 1024;
const MAX_VIDEO_BYTES = 15 * 1024 * 1024; // base64 adds ~33% on top of this
const ALLOWED_IMAGE_TYPES = ["image/jpeg", "image/png", "image/gif", "image/webp"];
const ALLOWED_VIDEO_TYPES = ["video/mp4", "video/webm", "video/quicktime"];

/** Rough byte size of a base64 data URL, without decoding the whole thing. */
